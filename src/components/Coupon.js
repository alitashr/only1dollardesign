...
import { checkCouponBalance, couponProvider, downloadLinkPrefix, getApiKey } from "../api/appProvider";

let errorMsgs = [
  "Must match the previous entry",
  "Coupon code is not valid",
  "Not enough coupon balance to checkout",
...
const checkCoupon = (code, total) => {
  return new Promise((resolve, reject) => {
    axios
      .post(checkCouponBalance + "?action=checkcoupon&id=" + code + "&total=" + total)
      .then((response) => {
        ...
        <CouponForm sm={{ span: 6 }}>
          <Form onSubmit={submitForm}>
            ...
            <div className="couponErrorMsg">
              {couponFormState.errorMsg !== "" ? <CouponMsg>{couponFormState.errorMsg}</CouponMsg> : null}
            </div>
            <div className="couponErrorMsg">{couponError !== "" && <CouponMsg>{couponError}</CouponMsg>}</div>
